...
const UserProfileForm = ({ userId,  }) => {
  ...
  const validatePersonalInfo = () => {
    const { firstName, lastName, email, contact, gender, dateOfBirth } =
      personalInfo;
    return (
      firstName.trim() !== "" &&
      lastName.trim() !== "" &&
  ...
  const fetchCommonAccountData = async () => {
    try {
      // Fetch account data from the server
      const response = await axios.get(
      ...
      setBankDetail(
        response.data.commonBankAccountDetais.filter((account) =>
          customer.commonBankAccounts.includes(account.iban)
        )
      );
    } catch (error) {
      // Handle error
  ...
  const handleSelectChange = (event) => {
    setCustomer({ ...customer, commonBankAccounts: event.target.value });
        ...
        return (
          <div className="text-color">
            ...
            <div className="row-2">
              ...
              <div className="col">
                <button className="btn btn-primary" onClick={handleChangeBank}>
                  Add Bank
                </button>
              </div>
            </div>
  ...
  return (
    <div className="mt-5">
      {customer.status === "Active" ? (
        ...
        <div className=" register card  card form-card border-color text-color custom-bg">
          <div className="alert alert-success" role="alert">
           <b> Your account is created successfully </b> 
