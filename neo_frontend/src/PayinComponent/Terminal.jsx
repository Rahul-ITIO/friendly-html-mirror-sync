...
import { FaEdit, FaTrash, FaEye, FaSync, FaCopy, FaFileExport, FaPlus, FaMinus, FaEyeSlash } from 'react-icons/fa';
...
const Terminal = () => {
  ...
  useEffect(() => {
    if (currentTerminal.merid) {
      fetchProcessingFeeOptions(currentTerminal.merid);
    }
  }, [currentTerminal.merid]);
  ...
  const fetchTerminals = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get(`/api/terminals/search?${params}`);
      if (response.data) {
  ...
  const validateForm = (payload) => {
    if (!payload.merid || !payload.connectorids) {
      toast.error("Merid and Connectorids are required");
      return false;
    }
    return true;
  };
  ...
  const generatePublicKey = async () => {
    try {
      if (!currentTerminal.id || !currentTerminal.merid) {
      ...
      <div className="container-fluid card py-4">
        ...
        <div className="row ">
            ...
            <div className="row g-3">
              ...
              <div className="col-md-2">
                <input
                  type="text"
                  className="form-control"
                  placeholder="Connector IDs"
                  name="connectorids"
                  value={searchCriteria.connectorids}
                  onChange={handleSearch}
                />
              </div>
              <div className="col-md-2">
              ...
              <div className="col float-end">
                    <div className="d-flex float-end">
                        <button className="btn btn-primary me-2" onClick={() => openModal()}>Add Terminal</button>
          ...
          <Modal 
            ...
            className="modal-dialog modal-xl p-4"
            ...
            <div className="modal-content">
              <form id="terminal-form" onSubmit={handleSubmit} className="row">
                <div className="modal-header" style={{ position: 'sticky', top: 0, backgroundColor: 'white', zIndex: 1 }}>
                  <h5 className="modal-title">
                      {currentTerminal?.id
                        ...
                        name="merid"
                        value={currentTerminal.merid}
                        onChange={handleInputChange}
                      >
                        <option className="mb-3" value="">Select MerID</option>
                        {Array.isArray(customers) && customers.length > 0 ? (
                          customers.map(customer => (
                            <option key={customer.id} value={customer.id}>
                              [{customer.id}] {customer.userName}{customer.name ? ` | ${customer.name}` : ''}
                ...
                <div className="modal-body" style={{ maxHeight: 'calc(90vh - 130px)', overflowY: 'auto' }}>
                  ...
                  <div className="row rounded text-dark">
                  <div className="col-md-6">
                    ...
                    <div className="col-md-6">
                      <label className="form-label">Connectorids:</label>
                      <input
                        type="text"
                        className="form-control"
                        name="connectorids"
                        value={currentTerminal.connectorids}
                        onChange={handleInputChange}
                      />
                    </div>
                    <div className="col-md-12">
                      <label className="form-label">Terminal JSON Value:</label>
                      <textarea className="form-control" name="ternoJsonValue" value={currentTerminal.ternoJsonValue} onChange={handleInputChange} />
                      <div className="col-md-12">
                        <div className="form-label">Json map for processing connector keys as per fee setting:
                          {/* View Button for this */}
                           <button 
                              onClick={(e) => handleExpandAll(e)} 
                             className="btn btn-sm ms-2 bg-white text-dark"
                              type="button"
                            >
                              {expandAll ? <FaMinus /> : <FaPlus />} {expandAll ? 'COLLAPSE' : 'EXPAND'}
                            </button>
                        </div>
                        <div className="border rounded p-2 row " style={{ overflowY: 'auto', backgroundColor:"#e1cef2" }}>
                          {processingFeeOptions.length > 0 ? (
                            processingFeeOptions.map((option) => (
                              <div className="col-md-6 p-0 pe-2" key={option.id}>
                                <div className="row align-bottom text-start p-0 m-0">
                                  {/* Checkbox confirm for are you sure to modify `[${option.id}] - ${option.connectorId}`  */}
                                  <div className="col-md-2 text-start" style={{ maxWidth: '50px' }}>
                                    <span className="form-check form-switch">
                                      <input
                                        className="form-check-input"
                                        type="checkbox"
                                        id={`processing-fee-${option.id}`}
                                        checked={selectedProcessingFees.includes(option.id.toString())}
                                        onChange={() => handleProcessingFeeCheckboxChange(option)}
                                      />
                                    </span>
                                  </div>
                                  <div className="col-md-7 text-start">
                                  <span>{`[${option.id}]`}</span> - <span className="fw-bold">{`${option.connectorId}`}</span>
                                    {option.connectorName && 
                                      <span className="small text-dark ps-2"> 
                                        - {option.connectorName} 
                                        {option.channelType && <span> | {option.channelType}</span>}
                                        {option.connectorStatus && 
                                          <span className={`ms-2 badge ${option.connectorStatus.toLowerCase() === 'active' ? 'bg-success' : 'bg-secondary'}`}>
                                            {option.connectorStatus}
                                          </span>
                                        }
                                        {option.defaultConnector && 
                                          <span className="ms-2 badge bg-primary">Default</span>
                                        }
                                      </span>
                                    }
                                  </div>
                                  <div className="col-md-3 ">
                                    ...
                                    {/* View Button for this */}
                                    <button
                                      className="btn bg-white text-dark btn-sm mt-1 mb-1 p-0 px-2 float-end"
                                      onClick={(e) => handleViewJson(option.id.toString(), e)}
                                      style={{ maxWidth: '50px' }}
                                      type="button"
                                      title={visibleJsonItems.includes(option.id.toString()) ? "Hide JSON" : "View JSON"}
                                    >
                                      {visibleJsonItems.includes(option.id.toString())
                                        ? <FaEyeSlash />
                                        : <FaEye />}
                                    </button>

                                    {/* Input for radio when click then value is {`${option.connectorId}`} for connectorids currentTerminal.connectorids */}
                                    <span className="form-radio form-switch float-end">
                                      <input  
                                        type='radio' 
                                        title='Starting s2s Connector No.'
                                        value={`${option.connectorId}`} 
                                        name="connectorids_radio" 
                                        className="form-check-input" 
                                        checked={currentTerminal.connectorids === `${option.connectorId}`}
                                        onChange={() => handleConnectorRadioChange(`${option.connectorId}`)}
                                      />
                                    </span>
                                  </div>
                                </div>
                                <div className="ps-1 " htmlFor={`processing-fee-${option.id}`}>

                                  {/* Modify for default_hide if expand_all from EXPAND or view_this from View Button for this */}
                                  <div
                                    className="col-md-6"
                                    style={isJsonItemVisible(option.id.toString()) ? styles.visible : styles.defaultHide}
                                  >
                                    <div className="this_editable_textarea_div">
                                      {selectedProcessingFees.includes(option.id.toString()) && (
                                        <textarea
                                          className="form-control this_editable_textarea"
                                          value={optionJsonValues[option.id.toString()] || option.terJsonDataValue || '{}'}
                                          onChange={(e) => handleJsonDataChange(option.id.toString(), e.target.value)}
                                          onKeyPress={handleKeyPress}
                                        />
                                      )}
                                    </div>
                                    {!selectedProcessingFees.includes(option.id.toString()) && (
                                      <span className="this_label_original_text_view">
                                        {option.terJsonDataValue}
                                      </span>
                                    )}
                                  </div>
                                </div>
                            ...
                            <p className="text-muted">No processing fee options available for this merchant</p>
                          ) : (
                            <p className="text-muted">Select a merchant to view processing fee options</p>
                          )}
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="checkbox_div col-sm-2" title="To Merchant on Approved">
                        <span className="form-check form-switch float-start">
                          <input 
                            ...
                            id="tarns_alert_email1" 
                            className="checkbox_d form-check-input"
                            value="001"
                            checked={selectedTarnsAlertEmails.includes('001')}
                            onChange={() => handleTarnsAlertEmailChange('001')}
                          />
                        </span>
                        <label className="form-check-label notification-text" htmlFor="tarns_alert_email1"><strong>Approved</strong></label>
                      </div>
                      ...
                      <div className="checkbox_div col-sm-2" title="To Merchant on Declined">
                        <span className="form-check form-switch float-start">
                          <input 
                            type="checkbox" 
                            name="tarns_alert_email[]" 
                            id="tarns_alert_email4" 
                            className="checkbox_d form-check-input"
                            value="004"
                            checked={selectedTarnsAlertEmails.includes('004')}
                            onChange={() => handleTarnsAlertEmailChange('004')}
                          />
                        </span>
                        <label className="form-check-label notification-text" htmlFor="tarns_alert_email4"><strong>Declined</strong></label>
                      </div>
                      ...
                      <div className="checkbox_div col-sm-2" title="To Merchant on Withdraw">
                        <span className="form-check form-switch float-start">
                          <input 
                            ...
                            className="checkbox_d form-check-input"
                            value="011"
                            checked={selectedTarnsAlertEmails.includes('011')}
                            onChange={() => handleTarnsAlertEmailChange('011')}
                          />
                        </span>
                        <label className="form-check-label notification-text" htmlFor="tarns_alert_email_011"><strong>Withdraw</strong></label>
                      </div>
                      ...
                      <div className="checkbox_div col-sm-2" title="To Merchant on Chargeback">
                        <span className="form-check form-switch float-start">
                          <input 
                            ...
                            className="checkbox_d form-check-input"
                            value="012"
                            checked={selectedTarnsAlertEmails.includes('012')}
                            onChange={() => handleTarnsAlertEmailChange('012')}
                          />
                        </span>
                        <label className="form-check-label notification-text" htmlFor="tarns_alert_email_012"><strong>Chargeback</strong></label>
                      </div>
                      ...
                      <div className="checkbox_div col-sm-2" title="To Merchant on Refund">
                        <span className="form-check form-switch float-start">
                          <input 
                            ...
                            className="checkbox_d form-check-input"
                            value="013"
                            checked={selectedTarnsAlertEmails.includes('013')}
                            onChange={() => handleTarnsAlertEmailChange('013')}
                          />
                        </span>
                        <label className="form-check-label notification-text" htmlFor="tarns_alert_email_013"><strong>Refund</strong></label>
                      </div>
                      ...
                      <div className="checkbox_div col-sm-2" title="Notification to customer for success and decline.">
                        <span className="form-check form-switch float-start">
                          <input 
                            type="checkbox" 
                            name="tarns_alert_email[]" 
                            id="tarns_alert_email2" 
                            className="checkbox_d form-check-input"
                            value="002"
                            checked={selectedTarnsAlertEmails.includes('002')}
                            onChange={() => handleTarnsAlertEmailChange('002')}
                          />
                        </span>
                        <label className="form-check-label notification-text" htmlFor="tarns_alert_email2"><strong>Customer</strong></label>
                      </div>
                    </div>
                  </div>
              ...
              <div className="modal-header" style={{ position: 'sticky', top: 0, backgroundColor: 'white', borderBottom: '1px solid #ccc', zIndex: 1 }}>
                <h5 className="modal-title p-3">
                          {currentTerminal?.id
                              ? (
                                  <>
                                      View Terminal Details: Terminal Number: <span style={{ color: '#dd00b0' }}><strong>{currentTerminal.id}</strong></span> |
                                      Merchant ID: <span style={{ color: 'green' }}><strong>{currentTerminal.merid}</strong></span>
                                  </>
                              )
              ...
              <div className="modal-body" style={{ maxHeight: 'calc(90vh - 130px)', overflowY: 'auto', padding: '20px' }}>
                <div className="row fs-6 gx-2">
                  {Object.entries(currentTerminal)
                    .filter(([key]) => !['id','merid','publicKeyDe','deletedBussinessUrl','ternoJsonValue'].includes(key)) // Exclude specified fields
                    .map(([key, value]) => (
                        ...
                        <div className="rounded p-2 bg-light">
                          {key === 'active' ? (value ? 'Yes' : 'No') : 
                           ...
                           (typeof value === 'object' && value !== null) ? (
                            <JsonPrettyPrint json={value} />
                          ) : typeof value === 'boolean' ? (
                            <span className="boolean-value">{String(value)}</span>
                          ) : value === null ? (
                  ...
                  <div className="col-md-12 mb-3">
                    <label className="fw-bold">Terno Json Value:</label>
                    <JsonPrettyPrint json={currentTerminal?.ternoJsonValue} />
                  </div>
                </div>
              </div>
            </div>
          ...
          >
            <div className="modal-content">
              <div className="modal-header bg-warning">
                <h5 className="modal-title p-2">Confirm Modification</h5>
              ...
              <div className="modal-body p-2">
                ...
                <p className="fw-bold">{confirmationOption ? `[${confirmationOption.id}] - ${confirmationOption.connectorId} - ${confirmationOption.connectorName}` : ''}</p>
              </div>
              <div className="modal-footer p-2">
                <button type="button" className="btn btn-secondary me-2" onClick={() => handleConfirmationResponse(false)}>Cancel</button>
