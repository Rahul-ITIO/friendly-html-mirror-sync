...
const QuickPay = () => {
  ...
  // State for storing beneficiary details from location state
  const [addBeneficiaryRequest, setaddBeneficiaryRequest] = useState(
    location.state
  );
   // Retrieve JWT token from session storage for authentication
  const customer_jwtToken = sessionStorage.getItem("customer-jwtToken");
  // State to manage recipient accounts, transfer request details, and various data
  ...
  const [calculatedFee, setCalculatedFee] = useState(0);
   // Retrieve active customer data from session storage
  const customer = JSON.parse(sessionStorage.getItem("active-customer"));
  // Hook to navigate programmatically
  let navigate = useNavigate();
  // Effect to fetch fee details when component mounts
  useEffect(() => {
    const getAllFeeDetails = async () => {
        // Retrieve fee details from server
      const feeResponse = await retrieveFeeDetails();
      if (feeResponse) {
         // Find fee details for "Debit Transaction" type
    ...
    const fetchAccounts = async () => {
      try {
        ...
        console.log(addBeneficiaryRequest);
        if (activeAccounts.length > 0) {
          setTransferRequest((prevState) => ({
            ...addBeneficiaryRequest,
            toBankAccount:addBeneficiaryRequest.accountNumber,
            toBankIfsc:addBeneficiaryRequest.swiftCode,
            country:addBeneficiaryRequest.country,
  ...
  const handleUserInput = async (e) => {
    ...
    if (name === "toCurrency") {
      ...
      const convertedAmount = (
        Number(transferRequest.amount) * exchangeRate
      ).toFixed(2);
      setTransferRequest((prevState) => ({
        ...prevState,
        fromCurrency: fromCurrency,
  ...
  const handleClick = () => {
    navigate("/customer/beneficiary/view");
  };
  return (
            ...
            <div className="row">
              ...
              <div className="col-md-4 card">
                ...
                <div className="mb-3">
                  ...
                  {errors.fromAccount && (
                    <div className="invalid-feedback">{errors.fromAccount}</div>
                  )}
                </div>
                <div className="mb-3">
                  <label className="form-label">
                ...
                <div className="mb-3">
                  ...
                  {errors.toCurrency && (
                    <div className="invalid-feedback">{errors.toCurrency}</div>
                  )}
                </div>
                <div className="mt-3 bg-light p-3 rounded">
                  ...
                  <p>
                    <b>Total Amount:</b>{" "}
                    {Number(calculatedFee) + Number(transferRequest.amount) ||
                      "N/A"}{" "}
                    {transferRequest.fromCurrency || ""}
                  </p>
                  <p>
                    <b>Converted Amount:</b> {transferRequest.toAmount || "N/A"}{" "}
